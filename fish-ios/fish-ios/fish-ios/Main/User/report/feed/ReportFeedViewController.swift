...
public struct ReportDateValueDetailItem {
...
class ReportFeedViewController:  FishPreViewController , UITableViewDelegate, UITableViewDataSource {
    ...
    var responseData:[MonthFeedDataModel] = []
    ...
    func loadData() -> Void {
        ...
        self.client.getMonthFeedData(params:["device_mac":self.selectedSceneModel?.deviceMac ?? "-",
...
//        self.navigationController?.pushViewController(detail, animated: true)
    ...
    func buildNetDataToTableData() {
        ...
        for item in self.responseData {
            tableList.append(ReportDateValueDetailItem.init(date: item.date ?? "-",
                                                       value: String(item.sum_actual_weight ?? 0)))
        }
    ...
    func buildNetDataToChartData(chartView: ReportLineChartView) {
        ...
        for (index, element) in self.responseData.enumerated() {
            ...
            values.append(ChartDataEntry.init(x: Double(index), y: element.sum_actual_weight ?? 0))
        }
...
//开启右滑返回手势:
extension ReportFeedViewController:HorizontalTitleItemViewDelegate {
