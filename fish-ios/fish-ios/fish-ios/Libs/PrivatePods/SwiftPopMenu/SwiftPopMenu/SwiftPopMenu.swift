...
public class SwiftPopMenu: UIView {
    ...
    public var popData:[(String)]! = [(String)](){
        didSet{
            //计算行高
...
//        self.iconImage.frame = CGRect(x: 10, y: (self.bounds.size.height - 20)/2, width: 20, height: 20)
        self.lblTitle.frame = CGRect(x: 10, y: 0, width: self.bounds.size.width - 20, height: self.bounds.size.height)
        self.line.frame = CGRect(x: 5, y: self.frame.size.height - 1, width: self.frame.size.width - 10, height: 1)
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if popData.count>indexPath.row {
            ...
            if indexPath.row == popData.count - 1 {
                cell.fill(title: model,textColor: popTextColor, islast: true)
            }else{
                 cell.fill(title: model,textColor: popTextColor)
            }
            return cell
        }
